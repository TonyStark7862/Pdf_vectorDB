...
from vector_database_fix import VectorDatabase  # Use the fixed vector database
        ...
        for i, result in enumerate(results):
            ...
            if 'metadata' in result and 'source' in result['metadata']:
                ...
                if 'pages' in result['metadata']:
                    source_info += f", Pages: {result['metadata']['pages']}"
                source_info += "\n"
            
            context_part = f"[Document {i+1}] {source_info}{result['text']}"
            context_parts.append(context_part)
        
        context_text = "\n\n".join(context_parts)
        
        # If no context was found, provide a note
