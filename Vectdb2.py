...
class VectorDatabase:
    ...
    def search(self, query: str, top_k: Optional[int] = None, cutoff_score: Optional[float] = None) -> List[Tuple[Chunk, float]]:
        ...
        # Search the index
        # Fix: faiss.search returns D (distances), I (indices) format
        ...
        for i in range(len(indices[0])):
            ...
            # Skip invalid indices and scores above cutoff 
            # (for L2 distance, lower is better, so we check for > cutoff)
            if idx == -1 or distance > cutoff_score * 10:  # Convert similarity threshold to distance threshold
                continue
            ...
            # For L2 distance, smaller is better, so we use an inverse relationship
            max_distance = 4.0  # Maximum expected L2 distance for normalized vectors
